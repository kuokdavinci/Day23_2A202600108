...
def predict(req: PredictRequest) -> PredictResponse:
    ...
    with instrumentation.tracer.start_as_current_span("predict") as span:
        ...
        try:
            ...
            with instrumentation.tracer.start_as_current_span("embed-text") as s:
                s.set_attribute("text.length", len(req.prompt))
                time.sleep(0.005)

            with instrumentation.tracer.start_as_current_span("vector-search") as s:
                s.set_attribute("k", 5)
                time.sleep(0.010)

            with instrumentation.tracer.start_as_current_span("generate-tokens") as s:
